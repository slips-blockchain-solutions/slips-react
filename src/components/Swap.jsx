...
import { useBlockNumber, useOnRepetition } from "eth-hooks";
import { abi as IUniswapV2Router02ABI } from '@uniswap/v2-periphery/build/IUniswapV2Router02.json'
import { useDebounce } from "../hooks";
import tokenListJson from '../tokenlists/slips-tokenlist.json';
import tokenTestListJson from '../tokenlists/testnet-tokenlist.json';
...
let defaultToken = 'BNB'
let defaultTokenOut = 'BUSD'
...
function Swap({ selectedProvider, selectedChainId }) {
  ...
  useEffect(() => {
    const getTokenList = async () => {
      ...
      let _tokens = tokenListToObject(filteredTokens)
      setTokens(_tokens)
    } catch (e) {
      console.log(e)
    }
    ...
    getTokenList()
  },[selectedProvider, selectedChainId, activeChainId, WETH])
  ...
  const getTrades = async () => {
    ...
    let baseTokens = tokenList.filter(function (t) {
      return ['WBNB', 'USDC', 'USDT', 'COMP', 'BNB', 'MKR', 'LINK', tokenIn, tokenOut].includes(t.symbol)
    }).map((el) => {
  ...
  const getAccountInfo = async () => {
    ...
    if(tokens && WETH && selectedProvider && tokens[tokenIn]) {
  ...
  useOnRepetition(getAccountInfo, 6000)
    ...
    try {
      ...
      if (exact === 'in') {
        ...
        if (tokenIn === 'BNB') {
          ...
          call = tokenOut === 'BNB' ? 'swapExactTokensForETH' : 'swapExactTokensForTokens'
        ...
        if (tokenIn === 'BNB') {
          ...
          call = tokenOut === 'BNB' ? 'swapTokensForExactETH' : 'swapTokensForExactTokens'
  ...
  return (
    <Card title={<Space><img src="https://ipfs.io/ipfs/QmXttGpZrECX5qCyXbBQiqgQNytVGeZW5Anewvh2jc4psg" width='40' alt='uniswapLogo'/><Typography>SLIP Swap</Typography></Space>} extra={<Button type="text" onClick={() => {setSettingsVisible(true)}}><SettingOutlined /></Button>}>
